...
alter table if exists tryout_questions disable row level security;
alter table if exists duel_questions disable row level security;
alter table if exists tryout_history disable row level security;
alter table if exists audit_logs disable row level security;
alter table if exists expenses disable row level security;
alter table if exists counseling_notes disable row level security;
alter table if exists helpdesk_tickets disable row level security;
alter table if exists live_session disable row level security;
`;

const body = JSON.stringify({ query: SQL });

const options = {
  hostname: "dhknzpelvgvwobbdyurj.supabase.co",
  path: "/rest/v1/rpc/exec",
  method: "POST",
  headers: {
    "Content-Type": "application/json",
    "apikey": "sb_publishable_W5pHgnLsPMyA-kVtFdQ3qA_nBkZ4yUN",
    "Authorization": "Bearer sb_publishable_W5pHgnLsPMyA-kVtFdQ3qA_nBkZ4yUN",
