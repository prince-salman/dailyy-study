...
export default function ExamPage() {
  ...
  useEffect(() => {
    ...
    const loadQuestions = async () => {
      const { data } = await supabase.from("tryout_questions").select("*");
      ...
      const shuffled = shuffleArray(questions).map((q: any) => {
        const optionsWithIndex = q.options.map((opt: string, idx: number) => ({ text: opt, isCorrect: idx === q.ans }));
        ...
        const cleanedOptions = shuffledOptions.map((o: any, i: number) => {
          const text = o.text.replace(/^[A-E]\.\s*/, "");
  ...
  const handleSubmit = async (forced = false) => {
    ...
    const score = Math.round((correctCount / Math.max(shuffledQuestions.length, 1)) * 1000);
    const userEmail = localStorage.getItem("userEmail") || "";

    await supabase.from("tryout_history").insert({
  ...
  const isPK = question?.subject?.includes("Kuantitatif");
        ...
        <div className="flex-grow flex flex-col lg:flex-row max-w-6xl mx-auto w-full p-4 lg:p-6 gap-6">
          <div className="flex-grow flex flex-col bg-slate-800 border border-slate-700 rounded-2xl p-6 shadow-xl relative">
            ...
            <div className="flex justify-between items-start mb-6 border-b border-slate-700 pb-4">
              <div>
                <div className="text-xs font-bold text-emerald-400 mb-1">{question.subject}</div>
            ...
            <div className="flex justify-between items-center mt-8 pt-6 border-t border-slate-700">
              ...
              {currentQ === shuffledQuestions.length - 1 ? (
                ...
                <button
                  onClick={() => setCurrentQ(Math.min(shuffledQuestions.length - 1, currentQ + 1))}
