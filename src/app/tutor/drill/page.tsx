...
export default function DrillQuestions() {
  ...
  useEffect(() => {
    ...
    if (saved.length > 0) {
      ...
      const init = [
        { id: 1, q: "Rumus kimia dari air adalah...", options: ["CO2", "H2O", "O2", "NaCl"], ans: 1 }
      ];
      setQuestions(init);
      localStorage.setItem("tutor_drill_questions", JSON.stringify(init));
    }
  }, []);
  ...
  const handleGenerateAI = () => {
    ...
    setTimeout(() => {
      const aiQs = [
        { id: Date.now() + 1, q: "Siapakah penemu bola lampu?", options: ["Thomas Edison", "Albert Einstein", "Nikola Tesla", "Isaac Newton"], ans: 0 },
        { id: Date.now() + 2, q: "Ibukota dari negara Jepang adalah...", options: ["Seoul", "Beijing", "Tokyo", "Kyoto"], ans: 2 }
      ];
  ...
  return (
    <div className="animate-in fade-in duration-300">
      <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-extrabold text-white">Bank Soal Drill</h1>
          <p className="text-slate-400 text-sm mt-1">Kelola soal untuk latihan cepat (Drill) harian siswa.</p>
        </div>
        ...
        <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-xl mb-8 animate-in slide-in-from-top-4">
          ...
          <div className="space-y-4">
                ...
                <div key={idx}>
                  ...
                  <div className="flex items-center gap-2">
                    <input 
                      ...
                      className="w-4 h-4 text-emerald-600 bg-slate-950 border-slate-800"
                    />
                    <input 
                      type="text" 
                      value={opt}
            ...
            <div className="pt-4 border-t border-slate-800 flex justify-end">
              <button onClick={handleSave} className="bg-emerald-600 hover:bg-emerald-500 text-white px-6 py-2 rounded-xl font-bold transition-colors">
                Simpan Soal
              </button>
            </div>
          </div>
      ...
      <div className="space-y-4">
        {questions.length === 0 ? (
          <div className="text-center py-12 bg-slate-900/50 rounded-2xl border border-slate-800/50">
            ...
            <p className="text-slate-400 font-semibold">Belum ada soal Drill.</p>
          </div>
        ) : (
          questions.map((q, i) => (
